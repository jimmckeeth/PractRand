...
> [!NOTE]
> This information applies to RNG algorithms used through either a 
> "Polymorphic" or a "LightWeight" interface. Those used through a "Raw" 
> interface only support a subset of the interface described here. 

## 1. You have installed and configured PractRand, right?
See [installation](installation.md).

## 2. Pick an RNG algorithm.  
You can look at [RNG_engines](RNG_engines.md) for information about which ones would 
...
> [!NOTE]
> Most seeding interfaces are not available on "raw" RNGs. See 
> [RNG_interface_variations](RNG_interface_variations.md) for details on raw RNGs.  
...
the RNG's constructor a parameter of `PractRand::SEED_AUTO` and it takes 
...
and you want to override its current state with an automatic seeding 
...
constructed and you want to override its current state with an 
...
```cpp
PractRand::RNGs::Polymorphic::hc256 rng(13);
// or
rng.seed(13);
```

**4c. seeding from another RNG:** You can seed one RNG from a second 
RNG by passing the seeder to the seedee's constructor or calling the 
seedee's `seed()` method with a pointer at the seeder as its parameter.  
...
additional functionality relating to seeding. See [RNG_entropy_pools](RNG_entropy_pools.md)
